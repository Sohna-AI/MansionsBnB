...
const EditSpot = () => {
  ...
  useEffect(() => {
    ...
    setIsLoaded(true);
  }, [dispatch, spotId, spotToEdit]);

  const [formData, setFormData] = useState({
    country: spotToEdit.country,
    address: spotToEdit.address,
    city: spotToEdit.city,
    state: spotToEdit.state,
    lat: spotToEdit.lat,
    lng: spotToEdit.lng,
    description: spotToEdit.description,
    name: spotToEdit.name,
    price: spotToEdit.price,
    previewImage: spotToEdit.previewImage,
    // imageOne: extraImages[0].url || '',
    // imageTwo: extraImages[1].url || '',
    // imageThree: extraImages[2].url || '',
    // imageFour: extraImages[3].url || '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    let updatedSpot = await dispatch(editSpot(spotPayload, spotId));

    if (updatedSpot) {
      navigate(`/spots/${spotId}`);
    }
  };
  if (isLoaded && spotToEdit) {
    return (
      <>
